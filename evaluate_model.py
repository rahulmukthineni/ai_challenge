...
def evaluate_model(label_folder, output_folder):
    ...
    # Evaluate the models.
    challenge_score,challenge_fpr = compute_challenge_score(label_outcomes, output_outcome_probabilities)
    ...
    # Return the results.
    return challenge_score,challenge_fpr, auroc_outcomes, auprc_outcomes, accuracy_outcomes, f_measure_outcomes, mse_cpcs, mae_cpcs
...
def compute_challenge_score(labels, outputs):
    ...
    if np.any(fpr <= max_fpr):
        ...
        max_index = np.argmax(tpr[indices])
        max_tpr = tpr[max_index]
        min_fpr=fpr[max_index]

    return max_tpr,min_fpr
...
if __name__ == '__main__':
    ...
    # Unpack the scores.
    challenge_score, challenge_fpr, auroc_outcomes, auprc_outcomes, accuracy_outcomes, f_measure_outcomes, mse_cpcs, mae_cpcs = scores

    # Construct a string with scores.
    output_string = \
        'Challenge Score: {:.3f}\n'.format(challenge_score) + \
        'FPR: {:.3f}\n'.format(challenge_fpr) + \
